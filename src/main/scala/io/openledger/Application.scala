...
import io.openledger.transaction.Transaction.{ResultingBalance, RejectAccounting, AcceptAccounting, TransactionResult}
...
class Application extends App {
  ...
  def transactionMessenger(transactionId: String, message: AccountingStatus): Unit = message match {
    case AccountingSuccessful(accountId, availableBalance, currentBalance, _) =>
