...
object Application extends App with JsonSupport {
  ...
  val transfer = Source(1 to iterations).map(_ => TransferRoundTrip(Random.shuffle(accounts)).generate()).mapConcat(identity)
  ...
  val merged: (Map[String, Int], Int) = Await.result(
    Source.combine(transfer, transferRev, authCap, authCapRev, authPcap, authPcapRev, authRev)(Merge(_))
      ...
      Consumer.plainSource(ConsumerSettings(
        ...
        valueDeserializer = new ByteArrayDeserializer).withBootstrapServers(kafkaBootstraps).withGroupId("simulator").withProperty(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "true").withProperty(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest"), Subscriptions.topics("openledger_outgoing"))
        .log(s"$n RESPONSES")
        .take(n)
        .map(consumerRecord => {
