...
case class AuthPartialCaptureReverse(participants: Seq[String]) extends SequenceGenerator {
  ...
  private val authCap: Map[String, Seq[TransactionRequest]] = {
    ...
    createPairs(participants).flatMap(pair => {
      ...
      Seq(
        TransactionRequest(
          Operation.Authorize(
            Authorize(entryCode = "AUTH_PARTCAP", transactionId = txnId, accountToDebit = pair._1, accountToCredit = pair._2, amount = 2)
          )
        ),
        TransactionRequest(
          Operation.Simple(
  ...
  override def toString: String = s"${transactions.count(r => r.operation.isAuthorize)} auths rotated among ${participants.size} accounts followed by ${transactions.count(r => r.operation.isSimple)} transfers and ${transactions.count(r => r.operation.isCapture)} partial captures and ${transactions.count(r => r.operation.isReverse)} reversals"
