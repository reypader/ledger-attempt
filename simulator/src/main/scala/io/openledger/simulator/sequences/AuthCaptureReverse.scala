...
case class AuthCaptureReverse(
      ...
      .map(pair => {
        ...
        authBehavior(
          ...
          Seq(
            ...
            EntryRequest(
              Operation.Capture(
                Capture(entryId = txnId, amountToCapture = Random.between(0.0, 3.0))
              )
            ),
            EntryRequest(
              Operation.Reverse(
  ...
  def authBehavior(
    ...
    Behaviors.receiveMessage {
      case EntryResult(_, status, _, _, _, _) if status == "PENDING" =>
        ...
        captureBehavior(entryId, sequence, queue, maxIteration, currentIteration, monitor)
  ...
  def captureBehavior(
    ...
    Behaviors.receiveMessage {
      case EntryResult(_, status, _, _, _, _) if status == "SUCCESS" =>
        monitor ! Done("CAPTURE", Instant.now().toEpochMilli - mark)
  ...
  def reverseBehavior(
    ...
    queue.offer(((entryId, context.self), sequence(2)))
    val mark = Instant.now().toEpochMilli
    Behaviors.receiveMessage {
  ...
  override def toString: String = s"$iterations iterations of (auth,capture,reversal) between ${pairs.size} pairs"
}
