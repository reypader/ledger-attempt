...
case class AuthCaptureRoundTrip(
      ...
      .map(pair => {
        ...
        authBehavior(
          ...
          Seq(
            ...
            EntryRequest(
              Operation.Capture(
                Capture(entryId = txnId, amountToCapture = 3)
              )
            )
          ),
          queue,
  ...
  override def toString: String = s"$iterations iterations of (auth,capture) between ${pairs.size} pairs"
}
