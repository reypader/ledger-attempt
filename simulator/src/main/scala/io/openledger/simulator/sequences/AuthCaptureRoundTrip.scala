...
case class AuthCaptureRoundTrip(participants: Seq[String]) extends SequenceGenerator {
  ...
  private val authCap: Map[String, Seq[TransactionRequest]] = {
    ...
    createPairs(participants).flatMap(pair => {
      ...
      Seq(
        TransactionRequest(
          Operation.Authorize(
            Authorize(entryCode = "AUTH_CAP_REV", transactionId = txnId, accountToDebit = pair._1, accountToCredit = pair._2, amount = 1)
          )
        ),
        TransactionRequest(
          Operation.Simple(
  ...
  private val transactions = authCap("AUTH") ++ authCap("SIMPLE") ++ authCap("CAPTURE")

  override def generate(): Seq[TransactionRequest] = transactions
  override def count(): Int = transactions.size
  override def toString: String = s"${transactions.count(r => r.operation.isAuthorize)} auths rotated among ${participants.size} accounts followed by ${transactions.count(r => r.operation.isSimple)} transfers and ${transactions.count(r => r.operation.isCapture)} full captures"
