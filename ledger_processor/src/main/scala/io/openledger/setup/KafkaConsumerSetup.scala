...
import io.openledger.domain.entry.Entry.{apply => _}
import io.openledger.kafka_operations.EntryRequest
...
object KafkaConsumerSetup {
  ...
  def apply(
      ...
      consumerActor: ActorRef[StreamOp]
  )(implicit
      system: ActorSystem[_],
      executionContext: ExecutionContext,
      materializer: Materializer,
      scheduler: Scheduler
  ) = new KafkaConsumerSetup(settings, coordinatedShutdown, consumerActor)

  case class KafkaConsumerSettings(
      processingTimeout: FiniteDuration,
      messagePerSecond: Int,
      topics: Set[String],
      kafkaSourceSettings: ConsumerSettings[String, Array[Byte]],
      kafkaComitterSettings: CommitterSettings
  )

}

class KafkaConsumerSetup(
    settings: KafkaConsumerSettings,
    coordinatedShutdown: CoordinatedShutdown,
    consumerActor: ActorRef[StreamOp]
)(implicit
  ...
  private val consumerFlow: RunnableGraph[DrainingControl[Done]] = Consumer
    .committableSource(settings.kafkaSourceSettings, Subscriptions.topics(settings.topics))
    .throttle(settings.messagePerSecond, 1.second)
    .map(consumerRecord =>
      StreamMessage(EntryRequest.parseFrom(consumerRecord.record.value()).operation, consumerRecord.committableOffset)
    )
