  ...
  "A Transaction" when {
    ...
    "Posting" must {
      ...
      "remain in Posting after DebitPostFailed and resume to Posted then RollingBackCredit immediately (reversal marked)" in {
        ...
        inSequence {
          ...
          stubResultMessenger expects CommandRejected(txnId, LedgerError.UNSUPPORTED_TRANSACTION_OPERATION_ON_CURRENT_STATE) once
        ...
        ackProbe.expectMessageType[TxnAck] shouldBe Ack
        reverseResult.events shouldBe Seq(ReversalRequested())
        reverseResult.stateOfType[ResumablePosting].actualState shouldBe givenState.asInstanceOf[Posting].copy(reversalPending = true)
    ...
    "Authorizing (auth only)" must {
      ...
      "transition to Pending then immediately to RollingBackDebit after DebitHoldSucceeded (reversal marked)" in {
        ...
        getResult.stateOfType[RollingBackDebit].transactionId shouldBe txnId
        getResult.stateOfType[RollingBackDebit].authorizedAmount shouldBe transactionAmount
